...
Скрипт необходимо удалять с сайта сразу, как только завершите процедуру поиска, во избежание запуска посторонними лицами. Если скрипт размещен на сервере более 8 часов назад, скрипт перестанет запускаться из браузера. Необходимо будет загрузить скрипт на сервер заново. При наличии прав на сервере, скрипт может удалить сам себя, для этого необходимо кликнуть по ссылке "delete" на странице.
