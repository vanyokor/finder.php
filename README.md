...
#### Стандартный
Активируется по умолчанию. Если текст отличается хотя бы одной заглавной или строчной буквой от искомого, текст игнорируется.

#### Нечувствительный к регистру символов
Запуск строгого поиска. Поиск текста без учета регистра.
...
В режимах Стандартный и Чувствительный, скрипт соблюдает лимит размера файла. Если файл превышает установленный лимит, он проигнорируется и не будет отсканирован скриптом. Это позволяет снизить нагрузку на сервер, ускорить поиск. По умолчанию, скрипт не сканирует файлы, больше 512 килобайт. Можно уменьшить лимит до 1 килобайта. Настройка полезна для ускорения поиска на крупных сайтах или PHP фреймворках.
...
Максимальная ширина интерфейса увеличивается, чтобы вместить результаты поиска в одну строчку. Полезен для беглого поиска среди большого количества результатов, когда нет необходимости вчитываться в код, и для режима отображения директорий.
